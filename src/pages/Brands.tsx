...
export default function Brands() {
  ...
  const { data: brands, isLoading } = useQuery({
    queryKey: ['brands', sortColumn, sortDirection, currentPage],
    queryFn: async () => {
      // name and created_at are server-sortable; portal name sorting handled client-side
      const effectiveCol = ['name', 'created_at'].includes(sortColumn) ? sortColumn : 'created_at';
  ...
  const createMutation = useMutation({
    mutationFn: async (data: { portal_id: string; name: string; logo_url: string | null; existing_customer_logic: boolean }) => {
      const { data: created, error } = await supabase.from('brands').insert([{ ...data, logo_url: data.logo_url || null }]).select('id, name').single();
      if (error) throw error;
      return created;
    },
  ...
  const updateMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: { portal_id: string; name: string; logo_url: string | null; existing_customer_logic: boolean } }) => {
      const { data: updated, error } = await supabase.from('brands').update({ ...data, logo_url: data.logo_url || null }).eq('id', id).select('id, name').single();
      if (error) throw error;
      return updated;
    },
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            ...
            <p className="text-muted-foreground mt-1">Manage brands within your portals</p>
          </div>
            ...
            <DialogContent>
              ...
              <form onSubmit={handleSubmit} className="space-y-4 mt-4">
                <div className="space-y-2">
                  ...
                  <Select value={formData.portal_id} onValueChange={(value) => setFormData({ ...formData, portal_id: value })}>
                ...
                <div className="space-y-2">
                  ...
                  <Input id="name" placeholder="My Brand" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="logo_url">Logo URL (optional)</Label>
                  <Input id="logo_url" placeholder="https://example.com/logo.png" value={formData.logo_url} onChange={(e) => setFormData({ ...formData, logo_url: e.target.value })} />
                </div>
                <div className="flex items-center justify-between p-3 rounded-lg border bg-muted/30">
                  ...
                  <Switch id="existing_customer_logic" checked={formData.existing_customer_logic} onCheckedChange={(checked) => setFormData({ ...formData, existing_customer_logic: checked })} />
                </div>
                <Button type="submit" className="w-full" disabled={isPending || !formData.portal_id}>
                  {isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : editingBrand ? 'Update Brand' : 'Create Brand'}
                </Button>
              </form>
            </DialogContent>
          </Dialog>
                  ...
                  <Table className="min-w-[520px]">
                    <TableHeader>
                      <TableRow>
                        ...
                        >
                          <div className="flex items-center gap-1">Brand <SortIcon column="name" /></div>
                        </TableHead>
                        <TableHead
                          className="cursor-pointer select-none hover:bg-muted/50"
                          onClick={() => toggleSort('portal_id')}
                        >
                          <div className="flex items-center gap-1">Portal <SortIcon column="portal_id" /></div>
                        </TableHead>
                        <TableHead
                          className="hidden sm:table-cell cursor-pointer select-none hover:bg-muted/50"
                          onClick={() => toggleSort('created_at')}
                        >
                          <div className="flex items-center gap-1">Created <SortIcon column="created_at" /></div>
                        </TableHead>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-3">
                              <Avatar className="w-8 h-8">
                                ...
                                <AvatarFallback className="bg-primary/10 text-primary text-xs">{brand.name.charAt(0)}</AvatarFallback>
                              </Avatar>
                              <span className="font-medium">{brand.name}</span>
                            </div>
                          </TableCell>
                          <TableCell className="text-muted-foreground">{brand.portals?.name}</TableCell>
